...
class TopUsersLastMonth < Report
  def initialize(config)
    super
    @users = {}
    @min_day = Date.today.prev_month - 1
  end
  ...
  def on_done
    open("reports/#{@config[:name]}/top_users_last_month.tsv", 'w') do |out|
      out.puts "user\ttotal"
